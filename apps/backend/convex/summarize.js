...
function inferRequiresUserInput(response) {
...
function parseResponsePayload(raw, fallbackSource) {
        ...
        try {
            ...
            if (summary) {
                return {
                    ...
                    summary,
                    requiresUserInput: typeof parsed.requiresUserInput === "boolean"
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a label generator. Given a user prompt sent to a coding AI assistant, respond with ONLY a 3-4 word summary that describes what the user wants. No punctuation, no quotes. Examples: 'Fix sidebar spacing', 'Add dark mode', 'Refactor auth logic', 'Debug API timeout'.",
                    },
                    {
                        role: "user",
                        content: prompt,
