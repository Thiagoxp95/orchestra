...
export const evaluateFilter = action({
  ...
  handler: async (_ctx, { filter, payload }): Promise<{ pass: boolean; reason: string }> => {
        ...
        messages: [
          {
            role: "system",
            content:
              `You are a webhook payload filter. The user has defined a condition in plain English. ` +
