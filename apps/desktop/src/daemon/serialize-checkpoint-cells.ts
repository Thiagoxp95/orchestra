...
// Fork of @xterm/addon-serialize 0.14.0 ANSI handler, with OSC8 and extended
// underline transitions. Keep upstream wrapping/BCE behavior and parity tests.
import type { IBuffer, IBufferCell, IBufferRange, Terminal } from '@xterm/headless'
import { extendedAttributes } from './checkpoint-attributes'
type IAttributeData = IBufferCell
...
abstract class BaseSerializeHandler {
  ...
  protected _serializeString(_excludeFinalCursorPosition?: boolean): string { return ''; }
}
...
class StringSerializeHandler extends BaseSerializeHandler {
  ...
  protected _nextCell(cell: IBufferCell, _oldCell: IBufferCell, row: number, col: number): void {
    ...
    if (isEmptyCell) {
      ...
      const extension = extendedAttributes(cell, this._terminal)
      if (styleChanged || extension !== this._extension) {
        this._currentRow += extension
  ...
  protected _serializeString(excludeFinalCursorPosition: boolean): string {
    ...
    return content + '\x1b]8;;\x1b\\';
  }
}


export function serializeCheckpointCells(terminal: Terminal): string {
  const serializeBuffer = (buffer: IBuffer) => new StringSerializeHandler(buffer, terminal).serialize({
    start: { x: 0, y: Math.max(0, buffer.length - 10_000 - terminal.rows) },
    end: { x: terminal.cols, y: buffer.length - 1 },
  }, false)
  let data = serializeBuffer(terminal.buffer.normal)
  if (terminal.buffer.active.type === 'alternate') data += '\x1b[?1049h\x1b[H' + serializeBuffer(terminal.buffer.alternate)
  return data
}
