...
// Summarizes user prompts into 3-4 word labels and classifies agent responses,
// with a cheap model over OpenRouter. The generic summarizers live in
// local-server/summarize.ts (they used to be Convex HTTP actions); the
// classifier here takes the user's own OpenRouter settings from the renderer.

import { DEFAULT_OPENROUTER_CLASSIFIER_PROMPT } from '../shared/types'
import * as local from './local-server/summarize'
...
export async function summarizePrompt(prompt: string): Promise<string> {
  ...
  return local.summarizePrompt(normalizedPrompt)
}
...
export async function summarizeResponse(response: string): Promise<ResponseSummaryResult> {
  return local.summarizeResponse(response)
}
