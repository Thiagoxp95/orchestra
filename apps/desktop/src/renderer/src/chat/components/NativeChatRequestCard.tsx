...
import type { NativeChatReply, NativeChatRequest } from '../../../../shared/native-chat'
import {
  buildNativeAnswers,
      ...
      {request.kind === 'approval' ? (
        <div className="mt-3 flex gap-2">
          <button
            type="button"
            disabled={busy}
            onClick={() => void respond({ requestId: request.id, decision: 'allow' })}
          ...
          <button
            type="button"
            disabled={busy}
            onClick={() => void respond({ requestId: request.id, decision: 'deny' })}
            className="rounded-lg border border-border px-3 py-1.5 text-xs text-foreground hover:bg-surface-hover disabled:opacity-40"
          >
            Deny
          </button>
        </div>
        ...
        <div className="mt-3 space-y-3">
                  ...
                  {question.options.map((option) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          setAnswers((current) => {
                            ...
                            return {
                              ...current,
                              [question.id]: { selected: nextSelected, freeText: '' },
                            }
                          })
                        }
                        className={cn(
                          'rounded-lg border px-2.5 py-1.5 text-left text-xs disabled:opacity-40',
                          selected
                      ...
                      >
                        ...
                        {option.description && (
                          <span className="ml-1 text-muted-foreground/70">— {option.description}</span>
                        )}
                      </button>
                    )
                  })}
          ...
          <button
            ...
            onClick={() =>
              void respond({ requestId: request.id, answers: buildNativeAnswers(answers) })
            }
            className="rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground disabled:opacity-40"
          >
