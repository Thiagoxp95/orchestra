...
import { Check, ChevronDown, ChevronRight, Clock, Copy, Image as ImageIcon, MessageCircle } from 'lucide-react'
import { cn } from './cn'
import { splitUserImageTokens, type DisplayBlock } from './chat-messages'
import { formatElapsed, type TimelineRow } from './chat-timeline'
...
/**
 * An AskUserQuestion record, t3code style: a one-line work-log row. The live
 * interaction (options, custom answer) lives in the composer's pending panel,
 * driven by snapshot.requests — the timeline only logs that it happened.
 */
export function QuestionRow({ block }: { block: Extract<DisplayBlock, { kind: 'question' }> }) {
  const submitted = block.result != null && block.result.isError !== true
  return (
    <div className="flex items-center gap-1.5 rounded-md px-0.5 py-0.5 select-none">
      <span className="flex size-5 shrink-0 items-center justify-center text-muted-foreground/65">
