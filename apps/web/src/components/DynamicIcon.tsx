...
// Brand logos ported verbatim from the desktop app (DynamicIcon.tsx) so the
// web sidebar matches Orchestra Electron pixel-for-pixel.

function ClaudeLogo({ size, color }: { size: number; color: string }) {
  return (
...
function OpenAILogo({ size, color }: { size: number; color: string }) {
  return (
...
const cache = new Map<string, ComponentType<{ size?: number; color?: string; className?: string }>>()

export function DynamicIcon({ name, size = 16, color, className }: DynamicIconProps) {
  // Built-in tokens (match desktop DynamicIcon).
  const [Icon, setIcon] = useState<ComponentType<{ size?: number; color?: string; className?: string }> | null>(
    () => cache.get(name) ?? null,
  )
  ...
  useEffect(() => {
    ...
    let cancelled = false
    import('hugeicons-react').then((mod) => {
      const Resolved = (mod as Record<string, unknown>)[name] as
  ...
  if (name === '__openai__' || name === '__codex__') return <OpenAILogo size={size} color={color ?? 'currentColor'} />
  if (name === '__cursor__') return <CursorLogo size={size} color={color ?? 'currentColor'} />
  if (name === '__terminal__')
    return <ComputerTerminal01Icon size={size} color={color} className={className} />

  if (Icon) return <Icon size={size} color={color} className={className} />
  // Fallback while the dynamic hugeicon loads (or for unknown names): terminal.
  return <ComputerTerminal01Icon size={size} color={color} className={className} />
}

/** Resolve the icon token for a session, mirroring the desktop's logic. */
export function sessionIconToken(processStatus: string, actionIcon?: string): string {
  if (processStatus === 'claude') return '__claude__'
  if (processStatus === 'codex') return '__openai__'
  if (processStatus === 'cursor') return '__cursor__'
  return actionIcon || '__terminal__'
}
