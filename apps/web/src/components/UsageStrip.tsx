 ...
 * shows). It lives in the space the home-indicator padding was already
 * reserving, so it costs no terminal height on a phone.
 *
 * Renders nothing until the desktop has mirrored at least one usable window —
 * an older desktop build never writes `usage`, and a cold start hasn't probed
 * yet. ActionBar takes the home-indicator padding back in that case.
 */
export function UsageStrip({ token }: { token: string }) {
  const state = useQuery(anyApi.remote.getRemoteState, { token }) as
    | { usage?: unknown }
    | null
    | undefined

  return <UsageStripView chips={selectUsageChips(state?.usage)} />
}

/** Presentational half — split out so it can be rendered without a Convex client. */
export function UsageStripView({ chips }: { chips: UsageChip[] }) {
  if (chips.length === 0) return null

  return (
    <div
      // Scrolls horizontally for the same reason ActionBar does: a scoped cap
      // (e.g. a per-model weekly) can push a provider past the phone's width,
      // and clipping the number that's about to block you is the worst outcome.
      // usage-strip (globals.css): hidden while the soft keyboard is up.
      className="usage-strip pb-home-indicator flex items-center gap-3 overflow-x-auto border-t border-border bg-sidebar px-2 pt-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
    >
      {chips.map((chip) => (
        <div
          key={chip.id}
