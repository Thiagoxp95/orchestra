...
function ClaudeLogo({ color }: { color: string }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" className="opacity-60 shrink-0">
      <path
...
function StatusIcon({ status, color }: { status: ProcessStatus; color: string }) {
  if (status === 'claude') {
    return <span title="Claude Code"><ClaudeLogo color={color} /></span>
  }
  if (status === 'codex') {
    return <span title="OpenAI Codex"><OpenAILogo color={color} /></span>
  }
  return <span title="Terminal"><ComputerTerminal01Icon style={{ color }} size={18} /></span>
}

export function SessionItem({ label, processStatus, isActive, accentColor, wsColor, onClick, onDelete }: SessionItemProps) {
  ...
  return (
    ...
    >
      <StatusIcon status={processStatus} color={txtClr} />
      <span className="text-sm truncate flex-1">{label}</span>
      <span
        onClick={(e) => { e.stopPropagation(); onDelete() }}
        className="opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
        style={{ color: light ? '#999' : '#6b7280' }}
      >
        ×
      </span>
    </button>
